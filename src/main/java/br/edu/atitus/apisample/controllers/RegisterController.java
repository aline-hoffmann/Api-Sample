...
public class RegisterController {
	...
	public ResponseEntity<RegisterEntity> createRegister(@RequestBody RegisterDTO registerDTO) throws Exception{
		...
		//Buscar Usuário cadastrado quando a autenticação estiver funcionando, pega-se o usuário autenticado
		...
		//Ou apenas colocar o 201 dentro dos parênteses do status
	}
	
	@GetMapping
	public ResponseEntity<List<RegisterEntity>> getRegisters() throws Exception{
		var registers = service.findAll();
		// Msm coisa que List<RegisterEntity registers = service.findAll();
		
		return ResponseEntity.ok(registers);
		// return ResponseEntity.status(200).body(registers); OUTRO JEITO
	}
